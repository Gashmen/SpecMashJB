...
def check_possible_to_add_all_inputs_in_one_row(max_size: float, list_with_diametrs_float: list):
    ...
    if len(list_with_diametrs_float) == 1:
        ...
        for diametr in list_with_diametrs_float:
            if diametr == list_with_diametrs_float[0] or diametr == list_with_diametrs_float[-1]:
                check_max_size -= diametr - 5
            else:
                check_max_size -= diametr - 10
    if check_max_size > 0:
    ...
    if len(list_with_diametrs_float) == 1:
        ...
        for count_diametr, diametr in enumerate(list_with_diametrs_float):
            ...
            # Если последний, то ставим вначале
                ...
                return_list.append([return_list[-1][0] + (list_with_diametrs_float[count_diametr-1]/2) + 5 + (list_with_diametrs_float[count_diametr] / 2),
                                    y0+(min_size / 2)])
            else:
                return_list.append([return_list[-1][0] + (list_with_diametrs_float[count_diametr-1]/2) + 5 + (list_with_diametrs_float[count_diametr] / 2),
                                    y0+(min_size / 2)])
...
def move_inputs_one_row(coordinate_insert_input:list, free_length:float,max_size:float ):
    ...
    if len(coordinate_insert_input) == 1:
        coordinate_insert_input[0][0] = max_size/2
    else:
        for count,coordinates in enumerate(coordinate_insert_input):
            coordinates[0] = coordinates[0] + free_length * ((count+1)/(len(coordinate_insert_input)+1))
...
if __name__ == "__main__":
    ...
    full_name_dim = {'': None, 'ВЗ-Н12': 31.9, 'ВЗ-Н16': 36.2, 'ВЗ-Н20': 39.6, 'ВЗ-Н25': 37.3, 'ВЗ-Н25/Р': 37.3,
                     ...
                     'ВЗ-Б50': 72.8, 'ВЗ-Б63': 84.3, 'ВЗ-Б75': 97.6, 'ВЗ-Б75А': 99.6, 'ВЗ-Б90': 112.6, 'ВЗ-Б90В': 112.6,
                     'ВЗ-П20': 39.6, 'ВЗ-П20А': 39.6, 'ВЗ-П20В': 39.6, 'ВЗ-П32': 53.6, 'ВЗ-П32А': 53.6, 'ВЗ-П32В': 53.6,
                     'ВЗ-К20': 42.6, 'ВЗ-К25': 49.6, 'ВЗ-К32': 59.2, 'ВЗ-К40': 75.8}
    shell_name = 'VP.161610'
    dict_with_inputs_on_side = {'А': ['ВЗ-Н25', 'ВЗ-Н25'], 'Б': ['ВЗ-Н25'], 'В': ['ВЗ-Н25', 'ВЗ-Н25'], 'Г': [], 'Крышка': []}
    ...
    # drill_surface = create_points_of_drill_surface(
    #     doc=ezdxf.readfile('C:\\Users\\g.zubkov\\PycharmProjects\\FinalProject\\src\\dxf_base\\DXF_BASE.dxf'),
    #     side='rightside',
    #     shell_name='VP.161610')
    #
    # full_name_dim = {'': None, 'ВЗ-Н12': 31.9, 'ВЗ-Н16': 36.2, 'ВЗ-Н20': 39.6, 'ВЗ-Н25': 47.3, 'ВЗ-Н25/Р': 47.3,
    #                  'ВЗ-Н32': 53.6, 'ВЗ-Н32/Р': 53.6, 'ВЗ-Н40': 64.9, 'ВЗ-Н50': 72.8, 'ВЗ-Н63': 84.3, 'ВЗ-Н75': 97.6,
    #                  'ВЗ-Н75А': 99.6, 'ВЗ-Н90': 112.6, 'ВЗ-Н90А': 119.6, 'ВЗ-Н90В': 119.6, 'ВЗ-Н100': 132.6,
    #                  'ВЗ-Н100А': 132.6, 'ВЗ-Н100В': 132.6, 'ВЗ-Н16-МР12': 36.2, 'ВЗ-Н20-МР15': 39.6,
    #                  'ВЗ-Н20-МР16': 40.6, 'ВЗ-Н20-МР18': 41.6, 'ВЗ-Н20-МР20': 42.6, 'ВЗ-Н20-МР22': 43.6,
    ...
    #
    # print(max_diametr_in_rectangle)
    #
    # xy01 = define_rectangle_size_for_inputs(dict_with_x_y_coordinates=drill_surface)
    #
    # print(xy01)
    #
    # check_possible_inputs = check_possible_to_add_all_inputs_in_one_row(
    #     max_size=max(xy01['xy1'][0] - xy01['xy0'][0], xy01['xy1'][1] - xy01['xy0'][1]),
    #     list_with_diametrs_float=[47.3, 39.6, 39.6])
    #
    # print(check_possible_inputs)
    # if check_possible_inputs:
    #     coordinate_diam = \
    #         paint_circle_one_row(min_size=min(xy01['xy1'][0] - xy01['xy0'][0], xy01['xy1'][1] - xy01['xy0'][1]),
    #                              list_with_diametrs_float=[47.3, 39.6, 39.6])
    #
    #     print(coordinate_diam)
